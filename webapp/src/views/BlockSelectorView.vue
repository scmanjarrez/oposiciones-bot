      ...
      >
        ...
        >
          <div class="flex items-start justify-between mb-3">
            ...
            <span
              class="text-xs font-bold bg-slate-700 group-hover:bg-emerald-600 text-slate-300 group-hover:text-white px-2 py-1 rounded-lg transition-colors"
            >
              {{ block.id }}
            </span>
          </div>
          <h2 class="font-semibold text-white text-sm mb-1">{{ BLOCK_SHORT[block.id] ?? block.name }}</h2>
          <p class="text-slate-400 group-hover:text-emerald-200 text-xs">{{ filteredCount(block) }} preguntas · {{
            block.topics.length }} temas</p>
        </button>
      </div>
    </div>
  </div>
