...
function selectAnswer(idx) {
  if (isExam.value) {
    // In exam mode just record the selection (toggle)
    if (quiz.currentAnswer === idx) {
      quiz.answer(null)
    } else {
      quiz.answer(idx)
    }
  } else {
    // In study mode: set answer (if not already answered)
...
// Show correct answer feedback in study mode
function answerClass(idx) {
  if (!studyAnswered.value) return 'bg-slate-800 border-slate-700 hover:border-slate-500 text-slate-200'
  if (idx === q.value.correct) return 'bg-green-900/60 border-green-500 text-green-200'
  if (idx === quiz.currentAnswer && idx !== q.value.correct) return 'bg-red-900/60 border-red-500 text-red-200'
  return 'bg-slate-800/50 border-slate-700 text-slate-400'
}

function examAnswerClass(idx) {
  const selected = quiz.currentAnswer === idx
  return selected
...
<template>
  <div class="min-h-screen bg-slate-900 text-slate-100 flex flex-col">
    ...
    <!-- Timer (exam only) -->
    <TimerBar v-if="isExam" :time-left="quiz.timeLeft" />
    ...
    <div class="flex-1 max-w-2xl mx-auto w-full px-4 py-6">
      ...
      <!-- Question header -->
      <div v-if="q" class="mb-5">
        <div class="flex flex-wrap items-center gap-2 mb-3">
          <!-- Status badge -->
          <StatusBadge :status="q.status" />
          <!-- Metadata chips -->
          <span v-if="q.block" class="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded">{{ q.block }}</span>
          <span v-if="q.topic" class="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded">T{{ q.topic }}</span>
          <span v-if="q.year" class="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded">{{ q.year }}</span>
        </div>
      ...
      <div v-if="studyAnswered && q.reason" class="bg-slate-800 border border-amber-600/50 rounded-xl p-4 mb-6 text-sm">
        <div class="text-amber-400 font-semibold mb-1">⚠️ Nota sobre esta pregunta</div>
        <p class="text-slate-300">{{ q.reason }}</p>
      </div>
      <div v-if="studyAnswered && q.correct_original !== null && q.correct_original !== undefined && q.correct_original !== q.correct"
      ...
      <div v-if="isExam" class="mt-4 text-center">
        <button @click="quiz.finishQuiz()" class="text-sm text-slate-500 hover:text-red-400 transition-colors underline">
